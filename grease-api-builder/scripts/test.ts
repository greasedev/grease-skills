...
async function getBrowserContextId(cdpUrl: string): Promise<string> {
	...
	if (!Array.isArray(stateData) || stateData.length === 0) {
		throw new Error(
			"No browser context found. Ensure Chrome is running with remote debugging enabled.",
		);
	}
...
async function runOpenCliCommand(
	...
	try {
		const { stdout, stderr } = await execAsync(`${cmdStr} 2>/dev/null`, {
			timeout: 60000
		});
...
function compareResults(
	...
	if (greaseCount !== opencliCount) {
		differences.push(
			`Count mismatch: GreaseAI=${greaseCount}, OpenCLI=${opencliCount}`,
		);
	}
	...
	if (sampleMatch) {
		...
		for (
			let i = 0;
			i < Math.min(greaseSample.length, opencliSample.length);
			i++
		) {
			...
			for (const key of keysToCompare) {
				...
				if (gVal !== oVal && gVal && oVal) {
					differences.push(
						`Item ${i + 1} "${key}": Grease="${gVal}" vs OpenCLI="${oVal}"`,
					);
				}
			}
		}
...
function writeTestLog(
	...
	const log: TestLog = {
		...
		params,
		success,
		data_count: Array.isArray(extractedData)
			? extractedData.length
			: extractedData
				? 1
				: 0,
		sample_data: Array.isArray(extractedData)
			? extractedData.slice(0, 5)
			: extractedData
				? [extractedData]
				: undefined,
		comparison: compare
...
async function main(): Promise<void> {
	...
	const cdpUrlValue =
		args.find((a) => a.startsWith("--cdp"))?.split("=")[1] ||
		process.env.CDP_URL ||
		"http://localhost:9222";
	...
	if (taskResult?.extractData) {
		...
		if (parsed && typeof parsed === "object" && "extractData" in parsed) {
			...
			if (Array.isArray(container.extractData)) {
				...
				console.log(
					`Extracted ${extractedData.length} items from extractData.`,
				);
			}
		...
		if (
			extractedData &&
			grease.output_schema &&
			grease.output_schema.length > 0
		) {
	...
	if (shouldCompare) {
		...
		const opencliResult = await runOpenCliCommand(
			site,
			command,
			params,
			grease.variables,
		...
		if (!extractedData) {
			...
			const greaseItems = Array.isArray(extractedData)
				? extractedData
				: [extractedData];
			compare = {
				...
				sampleMatch: false,
				differences: [
					`OpenCLI failed: ${opencliResult.error || "Unknown error"}`,
				],
			};
		}
	}
