...
async function locateElement(jsonFile: string, pauseStep: number, newTarget?: string) {
  ...
  if (pauseStep < 0 || pauseStep >= allActions.length) {
    console.error(`暂停步骤索引无效: ${pauseStep} (总步骤数: ${allActions.length})`);
    return;
  }
  ...
  if ('xpath' in targetAction) {
    delete (targetAction as Record<string, unknown>).xpath;
  }
  ...
  const task: BrowserTask = {
    ...
    status: "waiting",
    user_id: "debug-user",
    params: apiJson.variables?.reduce((acc: Record<string, unknown>, v: { name: string; test?: unknown; default?: unknown }) => {
  ...
  console.log(`原始target: ${(pauseAction.argument as Record<string, unknown>).target || 'N/A'}`);
  if (newTarget) {
  ...
  try {
    ...
    console.error("定位失败:", err instanceof Error ? err.message : String(err));
  }
...
if (!jsonFile) {
  console.log("用法: npx tsx scripts/locate-element.ts <json文件> <暂停步骤索引> [新target描述]");
  console.log("示例: npx tsx scripts/locate-element.ts clis/weibo/group-create.json 3");
  process.exit(1);
}
