...
export type QueryTweetsOptions = {
  startDate?: string;
  endDate?: string;
  keyword?: string;
  interest?: string;
...
export async function querySavedTweets(agent: DbAgent, options: QueryTweetsOptions = {}): Promise<SavedTweet[]> {
  const db = await getPortfolioDb(agent);
  ...
  // Apply interest filter first if specified
  if (options.interest) {
    const interestId = interestIdFor(options.interest);
    tweets = await table.where('interestId').equals(interestId).limit(limit * 10).toArray() as SavedTweet[];
  } else {
    tweets = await table.orderBy('savedAt').reverse().limit(limit * 10).toArray() as SavedTweet[];
  }

  console.log(`[querySavedTweets] Initial tweets count: ${tweets.length}`);
  console.log(`[querySavedTweets] Options:`, JSON.stringify(options));

  // Show sample createdAt values
  if (tweets.length > 0) {
    console.log(`[querySavedTweets] Sample createdAt values:`);
    tweets.slice(0, 3).forEach((t, i) => {
      console.log(`  ${i + 1}. "${t.createdAt}" -> parsed: ${new Date(t.createdAt || '').toISOString()}`);
    });
  }
  ...
  if (options.startDate) {
    ...
    console.log(`[querySavedTweets] startDate filter: "${options.startDate}" -> ${start.toISOString()}`);
    ...
    console.log(`[querySavedTweets] After startDate filter: ${tweets.length} (removed ${beforeCount - tweets.length})`);
  }
  if (options.endDate) {
    const end = new Date(options.endDate);
    end.setHours(23, 59, 59, 999);
    console.log(`[querySavedTweets] endDate filter: "${options.endDate}" -> ${end.toISOString()}`);
    ...
    console.log(`[querySavedTweets] After endDate filter: ${tweets.length} (removed ${beforeCount - tweets.length})`);
  }

  // Apply keyword filter (search in text)
  if (options.keyword) {
  ...
  // Apply author filter
  if (options.author) {
    const author = options.author.toLowerCase().replace(/^@/, '');
    tweets = tweets.filter(t =>
      t.author?.toLowerCase() === author ||
      t.author?.toLowerCase() === `@${author}`
    );
  }

  console.log(`[querySavedTweets] Final result: ${tweets.length} tweets`);
...
export async function getFirstInterestField(agent: DbAgent): Promise<{ id: string; topic: string } | null> {
  const db = await getPortfolioDb(agent);
  const fields = await db.table('interest_fields').orderBy('updatedAt').reverse().limit(1).toArray();
  if (!fields.length) return null;
  const field = fields[0] as { id: string; topic?: string };
  return { id: field.id, topic: field.topic || field.id };
}

export async function getAllInterestFields(agent: DbAgent): Promise<Array<{ id: string; topic: string }>> {
  const db = await getPortfolioDb(agent);
  const fields = await db.table('interest_fields').toArray();
  return fields.map((field: { id: string; topic?: string }) => ({
    id: field.id,
