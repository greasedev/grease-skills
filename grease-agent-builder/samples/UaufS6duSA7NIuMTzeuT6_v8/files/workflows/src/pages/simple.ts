...
interface NormalizedSource extends Source {
  ...
  existingInList?: ListKey; // Which list this KOL already exists in
}
...
let existingHandlesByListKey: Record<ListKey, Set<string>> = { core: new Set(), radar: new Set() };
...
function sourcesView(): string {
  ...
  const newSources = visibleSources.filter(s => s.state === 'new');
  const existingSources = visibleSources.filter(s => s.state === 'existing');
  const addedSources = visibleSources.filter(s => s.state === 'add');
  ...
  return `
    <section class="view">
      ...
      <div class="card" style="margin-bottom:18px">
        <h3>X.com 列表选择</h3>
        <p class="tiny" style="margin-top:8px">可以复用已有列表；复用时会显示列表里已存在的账号。</p>
      ...
      <div class="grid sources">
        ...
        <section class="grid">
          <div class="row" style="justify-content:space-between">
            ...
            <button class="primary" id="addAll" ${hasNewSources ? '' : 'disabled'}>${hasNewSources ? '关注全部推荐' : '已全部关注'}</button>
          </div>
          ${existingSources.length ? `<p class="tiny" style="margin-top:8px;margin-bottom:12px">已存在于列表 (${existingSources.length})</p>` : ''}
          ${existingSources.map(sourceCard).join('')}
          ${newSources.length || addedSources.length ? `<p class="tiny" style="margin-top:16px;margin-bottom:12px">待添加 (${newSources.length + addedSources.length})</p>` : ''}
          ${(newSources.length || addedSources.length) ? (newSources.length ? newSources.map(sourceCard).join('') : '') + (addedSources.length ? addedSources.map(sourceCard).join('') : '') : ''}
          ${!visibleSources.length ? '<p class="muted" style="text-align:center;margin-top:24px">暂无推荐来源</p>' : ''}
        </section>
...
async function applyDuplicateFilterForSelectedLists(showLoading = true) {
  ...
  try {
    existingHandlesByListKey = await readExistingHandlesByListKey();

    // Build a map of handle -> listKey where it exists
    const handleToExistingListKey = new Map<string, ListKey>();
    for (const key of listKeys) {
      for (const handle of existingHandlesByListKey[key]) {
        handleToExistingListKey.set(handle, key);
      }
    }

    // Process sources: mark existing ones, filter duplicates across lists
    model.sources = allRecommendedSources
      .map(source => {
        const username = cleanHandle(source.handle || source.name).toLowerCase();
        const sourceType = source.type.toLowerCase() as ListKey;
        const existingListKey = handleToExistingListKey.get(username);

        if (existingListKey) {
          // This handle exists in a reused list
          if (existingListKey === sourceType) {
            // Exists in the SAME list it's recommended for -> mark as existing
            return { ...source, state: 'existing' as const, existingInList: existingListKey };
          } else {
            // Exists in a DIFFERENT list -> should be ignored (won't add to this list)
            return { ...source, state: 'ignore' as const };
          }
        }
        // Not in any reused list -> new recommendation
        return { ...source, state: 'new' as const };
      });

    picked = emptyPicked();
    // Auto-add new sources and existing sources
    model.sources.filter(s => s.state === 'new' || s.state === 'existing').forEach(addSource);
...
function sourceCard(source: NormalizedSource): string {
  // Show status badge
  const statusBadge = source.state === 'existing'
    ? `<span class="pill existing">已在列表</span>`
    : source.state === 'add'
      ? `<span class="pill added">已加入</span>`
      : '';

  const actions = source.state === 'existing'
    ? `<p class="ok muted">✓ 已在 ${capitalize(source.existingInList || source.type.toLowerCase() as ListKey)} List</p>`
    : source.state === 'add'
...
function addSource(source: NormalizedSource) {
  ...
  // Check if already added to another list
  for (const otherKey of listKeys) {
    if (otherKey !== key && picked[otherKey].some(item => {
      const existingHandle = cleanHandle(item.handle || item.name || '').toLowerCase();
      const newHandle = cleanHandle(source.handle || source.name).toLowerCase();
      return existingHandle === newHandle;
    })) {
      // Already in another list, don't add
      return;
    }
  }
  ...
  if (!picked[key].some(item => item.id === source.id)) {
    ...
    source.state = source.state === 'existing' ? 'existing' : 'add';
  }
}
